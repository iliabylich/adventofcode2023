...
impl Matrix {
    fn predict(&mut self) {
        ...
        for row_idx in (0..self.rows.len() - 1).rev() {
            let a = *self.rows[row_idx].numbers.last().unwrap();
            let b = *self.rows[row_idx + 1].numbers.last().unwrap();
            self.rows[row_idx].numbers.push(a + b);
        }
    }

    fn prediction(&self) -> i64 {
        *self.rows[0].numbers.last().unwrap()
    }
}
...
#[test]
fn test() {
    assert_eq!(solve(include_str!("problem1-input-small.txt")), 114);
}
